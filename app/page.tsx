...
export default function Home() {
  ...
  const [mastered,setMastered]=useState<string[]>([]); const [showLibrary,setShowLibrary]=useState(false);
  ...
  function startLibrary(){setShowLibrary(true);setTimeout(()=>document.getElementById('library')?.scrollIntoView({behavior:'smooth'}),50)}
  return <main>
    ...
    <section className="hero" id="top"><div className="hero-copy"><p className="eyebrow"><span>✦</span> Make words your superpower</p><h1>See it. Hear it.<br/><em>Say it!</em></h1><p className="intro">Build a world of words with playful pictures, clear pronunciation, and little wins every day.</p><div className="hero-actions"><button className="primary-button" onClick={startLibrary}>Start learning <span>→</span></button><button className="sound-button" onClick={()=>speak('Welcome to WordBloom. Let us learn together!')}><span>🔊</span> Try the sound</button></div><div className="mini-proof"><div className="faces"><span>🧒</span><span>👧</span><span>👦</span></div><p><strong>{allCards.length.toLocaleString()} picture words</strong><br/>ready to explore</p></div></div>
      <div className="practice-card" id="learn" style={{'--card-color':item.color} as React.CSSProperties}><div className="card-topline"><span>Try a word</span><span>{(active%filtered.length)+1} / {filtered.length}</span></div><div className="picture-stage"><span className="spark one">✦</span><span className="spark two">✦</span><span className="main-emoji" role="img" aria-label={item.label}>{item.emoji}</span></div><div className="word-row"><div><p className="word">{item.label}</p><p className="phonetic">Tap to listen and repeat</p></div><button className="speak-round" onClick={()=>speak(`${item.label}. ${item.sentence}`)} aria-label={`Hear ${item.label}`}>🔊</button></div><button className="sentence" onClick={()=>speak(item.sentence)}><span>▶</span> “{item.sentence}”</button><div className="card-controls"><button onClick={()=>pickNext(-1)} aria-label="Previous word">←</button><button className={mastered.includes(item.id)?'mastered':''} onClick={toggleMastered}>{mastered.includes(item.id)?'★ Learned':'☆ I learned it'}</button><button onClick={()=>pickNext(1)} aria-label="Next word">→</button></div></div>
    </section>
